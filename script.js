...
const weatherApi = {
    key: '11b298b30122c6fe932356e5742000db',
...
// add an event listener to the input box for when the user hits enter or clicks the search button
...
function getWeather(city) {
    ...
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=11b298b30122c6fe932356e5742000db&units=metric`)
        .then(response => response.json())
        .then(data => {
            // check if the temperature is more than 35 degrees
            if (data.main.temp > 35) {
                // show a prompt to turn on the irrigation system
                swal({
                    title: "Temperature Alert",
                    text: "The temperature is more than 35 degrees. Turn on your irrigation system?",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                })
                .then(response => {
                    if (response) {
                        prompt("temperature increased turn on the irrigation system ");
                    }
                });
            }
